...
class _netD(nn.Module):
    ...
    def _initialize_fc(self, conv6_output_size):
        # 根据conv6的输出动态定义全连接层
        fc_input_size = conv6_output_size[1] * conv6_output_size[2] * conv6_output_size[3]
        self.fc_dis = nn.Linear(fc_input_size, 1)
        self.fc_aux = nn.Linear(fc_input_size, 10)
    ...
    def forward(self, input):
        ...
        # 动态初始化全连接层
        if self.fc_dis is None or self.fc_aux is None:
            self._initialize_fc(conv6.size())

        print(f"Shape after conv6: {conv6.shape}")  # 打印conv6的输出形状

        flat6 = conv6.view(conv6.size(0), -1)
        print(f"Shape after view: {flat6.shape}")  # 打印view之后的形状
