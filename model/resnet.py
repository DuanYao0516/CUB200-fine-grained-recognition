...
class ResNet(nn.Module):
    def __init__(
        ...
        layers: List[int],
        num_classes: int = 1000,            #修改输出类别数量
        zero_init_residual: bool = False,   #是否将最后一层的 BN 初始化为零。
        
        #分组卷积
        groups: int = 1,                    #组数，用于分组卷积
        width_per_group: int = 64,          #每个分组的通道数。
        replace_stride_with_dilation: Optional[List[bool]] = None,  #是否用扩张卷积替换步幅。
        norm_layer: Optional[Callable[..., nn.Module]] = None,#标准化层的类型
    ) -> None:
        ...
        self.fc = nn.Linear(512 * block.expansion, num_classes)  #随机初始化fc层参数？？
    

        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                #使用 Kaiming 初始化方法（Kaiming Initialization）初始化权重。
                nn.init.kaiming_normal_(m.weight, mode="fan_out", nonlinearity="relu")
            elif isinstance(m, (nn.BatchNorm2d, nn.GroupNorm)):
                #分别将权重初始化为 1，偏置初始化为 0。
        ...
        #一个对残差分支中的最后一个批标准化层（Batch Normalization，BN）进行零初始化的操作。这样做的目的是确保残差分支的初始输出接近零，使得每个残差块的初始行为类似于一个恒等映射（identity mapping）
...
def _resnet(arch, pretrained=False,transfer_type=None,  progress=True, **kwargs):#**kwargs接收可变量的参数
    model = ResNet(**kwargs)
    #print(transfer_type)
    if pretrained:
        # if type ==None:
        #     print('Error,please input the type of transfer learning.')
        state_dict = load_state_dict_from_url(model_urls[arch],
                                              progress=progress)        #加载预训练的参数,progress显示加载槽
        # 修改全连接层参数的维度
        in_features = model.fc.in_features  # 获取全连接层原始输入特征数
        out_features = model.fc.out_features # 修改为新的输出特征数
        state_dict['fc.weight'] = state_dict['fc.weight'][:out_features, :in_features]  # 调整权重参数维度
        state_dict['fc.bias'] = state_dict['fc.bias'][:out_features]  # 调整偏置参数维度

        # 手动设置全连接层参数为随机初始化
        for name, param in model.named_parameters():
            if 'fc' in name:  # 找到全连接层参数
        ...
        if transfer_type==__transfer__[1]:
               ...
               #dont_freeze_layers=list(model.children())[:dont_freeze_num]
...
def resnet18(pretrained=False, transfer_type=None, progress=True, **kwargs):
    ...
    return _resnet('resnet18',
                   pretrained, transfer_type,progress,
                   block=BasicBlock,
                   layers=[2, 2, 2, 2],
                   **kwargs)

def resnet34(pretrained=False,transfer_type=None,  progress=True, **kwargs):
...
def resnet50(pretrained=False, transfer_type=None, progress=True, **kwargs):
...
def resnet101(pretrained=False,transfer_type=None, progress=True, **kwargs):
...
def resnet152(pretrained=False,transfer_type=None,  progress=True, **kwargs):
...
def resnext50_32x4d(pretrained=False, transfer_type=None, progress=True, **kwargs):
...
def resnext101_32x8d(pretrained=False, transfer_type=None,progress=True, **kwargs):
...
def resnext101_64x4d(pretrained=False, transfer_type=None, progress=True, **kwargs):
...
def wide_resnet50_2(pretrained=False, transfer_type=None, progress=True, **kwargs):
...
def wide_resnet101_2(pretrained=False, transfer_type=None, progress=True, **kwargs):
